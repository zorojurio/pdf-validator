...
class PublicKeyExtractor:
    ...
    def extract_public_key_from_p7c(self):
        ...
        pbk, pbk_str = PdfSignatureValidator.get_public_key_from_certification(certificate)
    ...
    def extract_public_key_from_fdf(self):
        ...
        edited = cert_data.replace(b'\\r', b'\r').replace(b'\\n', b'\n').replace(b'\\\\', b'\\').replace(
            b'\\\r', b'').replace(b'\\(', b'(').replace(b'\\)', b')')
